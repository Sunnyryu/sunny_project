...
def anal(request):
    ...
    date1 = date1[0:10]
    keyword_all = Keyword.objects.filter(word=keyword)
    for keyword5 in keyword_all:
        if keyword5.word_set.all().exists():
            word9 = keyword5.word_set.values('word')
        else:
            avg = 0
    #keyword_word = keyword_all.word
    #keyword_date = keyword_all.date
    #keyword_hwf = keyword_all.high_word_frequency
    if date1==keyword_date and keyword==keyword_word:
