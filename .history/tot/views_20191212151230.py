...
def anal(request):
    ...
    c = json.dumps(keyword2, ensure_ascii=False)
    strkeyword2 = json.dumps(list(keyword2), cls=DjangoJSONEncoder, ensure_ascii=False)
    replacestrkeyword2 = strkeyword2.replace("\\", "")
    replacestrkeyword = replacestrkeyword2.replace('""' , "")
    print(type(replacestrkeyword2))
    print(replacestrkeyword2)
    print(c)
    #keyword_all = Keyword.objects.filter(word=keyword, date=date1)
    #keyword_all2 = keyword_all.values()
    #keyword_all3 = serializers.serialize('json', keyword_all)

    #print(keyword_all)
    #keyword_word = keyword_all.word
    #keyword_date = keyword_all.date
    #keyword_hwf = keyword_all.high_word_frequency
    if date1 == 0:
        context = {'keyword_word':keyword_all.word}
